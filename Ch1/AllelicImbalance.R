...
bam_folder <- file.path(getwd(), "R_cook", allele_expression")

reads <- impBamGAL(bam_folder, region_of_interest, verbose = FALSE)

vcf_file <-file.path( getwd(), "R_cook", "allele_expression","ERP000101.vcf" )
...
reference_sequence <- file.path(getwd(), "R_cook","allele_expression", "hg19.chr17.subset.fa")
